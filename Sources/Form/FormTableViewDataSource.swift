...
final class FormTableViewDataSource: NSObject {
    ...
    func rowModel(at indexPath: IndexPath) -> FormTableViewModel? {
        return viewModel?[indexPath.row]
    }
    
    
    func setRowModel(_ aViewModel: FormTableViewModel, at indexPath: IndexPath) {
        viewModel?[indexPath.row] = aViewModel
    }
}
